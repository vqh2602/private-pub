...
For local development, you can use your system-level Dart/Flutter SDK (default) or FVM (optional, by setting `USE_FVM=true` or defining `FVM_EXECUTABLE` in your `.env`). This repository pins Flutter in `.fvmrc` (pinned toolchain is FVM `3.2.1`, Flutter `3.41.9`, and Dart `3.11.5`).

To activate the CLI locally and sign in with OAuth Authorization Code + PKCE:

Set `FLUTTER_VERSION` in `.env` and rebuild the image to change Docker's SDK. For local development with FVM, run `fvm use <version>` at the repository root. The Web footer reports the versions detected from the running toolchain.

...
const knownChannels: Exclude<ReleaseChannel, "stable" | "prerelease">[] = [
  "dev",
  "alpha",
  "beta",
  "rc",
...
export function releaseChannel(version: string): ReleaseChannel {
  ...
  return (
    knownChannels.find((channel) =>
      labels.some((label) => label === channel || label.startsWith(channel)),
    ) ?? "prerelease"
  );
}
...
export function selectLatestRelease<
  T extends { version: string; retractedAt?: unknown },
>(versions: T[]): T | undefined {
  ...
  const stable = candidates.filter(
    (version) => releaseChannel(version.version) === "stable",
  );
  return [...(stable.length ? stable : candidates)].sort((a, b) =>

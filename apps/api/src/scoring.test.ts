...
describe("automatic package scoring", () => {
  it("calculates a complete score breakdown from package contents", () => {
    const score = calculatePackageScore({
      ...
      files: [
        { path: "LICENSE" },
        { path: "lib/healthy_package.dart" },
        { path: "analysis_options.yaml" },
        { path: "test/healthy_package_test.dart" },
  ...
  it("deducts points for missing quality signals and risky dependencies", () => {
    ...
    expect(
      score.breakdown.find((item) => item.label === "Dependency health")
        ?.points,
    ).toBe(20);
    expect(
      score.breakdown.find((item) => item.label === "Dependency health")
        ?.details,
    ).toContain("local_dep uses a local path dependency (−10 points).");
    expect(
      score.breakdown.find((item) => item.label === "Documentation")?.details,
    ).toContain(
  ...
  it("deducts points based on analyzer findings (errors and warnings)", () => {
    ...
    const analysisBreakdown = score.breakdown.find(
      (item) => item.label === "Static analysis readiness",
    );
    expect(analysisBreakdown?.points).toBe(25);
    expect(analysisBreakdown?.details).toContain(
      "Analyzer error in lib/healthy_package.dart:10: Missing return ([missing_return] −12 points).",
    );
    expect(analysisBreakdown?.details).toContain(
      "Analyzer warning in lib/healthy_package.dart:25: Avoid print ([avoid_print] −3 points).",
    );
  });
});

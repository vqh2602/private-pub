...
export async function registerRoutes(app: FastifyInstance, repository: RegistryRepository) {
  ...
  app.post("/v1/auth/login", { config: { rateLimit: { max: 10, timeWindow: "1 minute" } } }, async (request, reply) => {
    ...
    return reply.setCookie(sessionCookieName, session.plaintext, sessionCookieOptions(session.expiresAt)).send({
      user: {
        id: account.id,
  ...
  app.get("/v1/auth/me", { preHandler: scopes("packages:read") }, async (request) => ({
    user: {
      id: request.actor!.id,
  ...
  app.get("/api/packages/:name/versions/:version.tar.gz", async (request, reply) => {
    ...
    return reply.code(501).send({
      error: {
        ...
        message: "Configure S3ArchiveStore for binary archives. Metadata and publish flow are available in demo mode.",
      },
    });
  });
      ...
      return reply
        ...
        .send({
          error: {
            ...
            message: "Expected multipart field 'file' containing a .tar.gz archive.",
          },
        });
      ...
      return reply
        ...
        .send({
          error: {
            ...
            message: "No uploaded archive was found for this publish session.",
          },
        });
    if (pending.actor.id !== request.actor!.id)
      return reply
  ...
  app.get("/v1/search", async (request) => {
    ...
    return {
      items: packages.slice((query.page - 1) * query.limit, query.page * query.limit),
  ...
  app.get("/v1/packages/:name/versions/:version/files", async (request, reply) => {
    ...
    return {
      package: name,
      version,
      entries: (await repository.getFiles(name, version)) ?? reply.code(404).send({ error: "not_found" }),
    };
  });
  app.get("/v1/packages/:name/versions/:version/files/*", async (request, reply) => {
  ...
  app.post("/v1/imports/file", { preHandler: scopes("imports:write") }, async (request, reply) => {
    ...
    try {
      ...
      return reply.code(201).send({
        ...
        action,
        message: published.packageCreated ? `Đã xác minh và tạo package ${published.name} ${published.version}.` : `Đã xác minh và thêm version ${published.version} vào package ${published.name}.`,
      });
    } catch (error) {
  ...
  app.post("/v1/analyses/recompute", { preHandler: scopes("packages:admin") }, async (request, reply) =>
    reply.code(202).send({
  ...
  app.get("/v1/tokens", { preHandler: scopes("tokens:write") }, async (request) => ({

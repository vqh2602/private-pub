...
export async function registerRoutes(app: FastifyInstance, repository: RegistryRepository) {
  ...
  app.get("/api/packages/:name", async (request, reply) => {
    ...
    if (!detail) return reply.code(404).send({ error: { code: "not_found", message: `Package ${name} was not found.` } });
    return {
      name,
      latest: detail.latestVersion,
      versions: detail.versions.map((item) => ({
        version: item.version,
        pubspec: { name, version: item.version, description: detail.package.description, environment: { sdk: item.sdk.dart } },
  ...
  app.get("/api/packages/:name/versions/:version", async (request, reply) => {
    ...
    const detail = await repository.getPackage(name);
    if (!item || !detail) return reply.code(404).send({ error: { code: "not_found", message: "Package version was not found." } });
    return { version, pubspec: { name, version, description: detail.package.description, environment: { sdk: item.sdk.dart } }, archive_url: `${request.protocol}://${request.host}/api/packages/${name}/versions/${version}.tar.gz`, archive_sha256: item.archiveSha256, published: item.publishedAt };
  });
  ...
  app.get("/api/packages/versions/newUpload", { preHandler: requireScopes("packages:publish") }, async (request) => {
    const uploadId = randomUUID();
    return { url: `${request.protocol}://${request.host}/api/uploads/${uploadId}`, fields: { uploadId }, finalizationUrl: `${request.protocol}://${request.host}/api/packages/versions/newUploadFinish?uploadId=${uploadId}` };
  });
  app.post("/api/uploads/:uploadId", { preHandler: requireScopes("packages:publish") }, async (_request, reply) => reply.code(202).send({ accepted: true, note: "Demo upload adapter accepted the request; connect S3ArchiveStore for persistence." }));
  app.get("/api/packages/versions/newUploadFinish", { preHandler: requireScopes("packages:publish") }, async (request) => ({ success: { message: `Upload ${String((request.query as { uploadId?: string }).uploadId ?? "unknown")} finalized and queued for validation.` } }));

...
export class DemoRegistryRepository implements RegistryRepository {
  ...
  async search(query: string, filters: { sdk?: string; platform?: string; publisher?: string; hasPreview?: boolean; sort?: string }) {
    ...
    let values = [...this.details.values()].map((entry) => entry.package).filter((item) => {
      ...
      return terms.every((term) => text.includes(term)) &&
        ...
        (!filters.platform || this.details.get(item.name)?.latestVersion.platforms.includes(filters.platform));
    });
  ...
  async getPackage(name: string) { return structuredClone(this.details.get(name) ?? null); }
  async getVersion(name: string, version: string) { return structuredClone(this.details.get(name)?.versions.find((item) => item.version === version) ?? null); }
  async getFiles(name: string, version: string) { return (await this.getVersion(name, version)) ? structuredClone(this.details.get(name)?.files ?? []) : null; }
  ...
  async getArchive(name: string, version: string) { return this.archives.get(`${name}@${version}`) ?? null; }
}

...
export class DemoRegistryRepository implements RegistryRepository {
  ...
  async initialize() {
    ...
    const filenames = (await readdir(directory)).filter((filename) => filename.endsWith(".tar.gz")).sort();
  ...
  async getStats() {
    ...
    const versions = details.reduce((total, detail) => total + detail.versions.length, 0);
    const analyzedVersions = details.reduce((total, detail) => total + (detail.score.breakdown.length > 0 ? detail.versions.length : 0), 0);
    ...
    let values = [...this.details.values()]
      ...
      .filter((item) => {
        ...
        return (
          ...
          (filters.verifiedPublisher === undefined || (item.publisherId !== "local.private") === filters.verifiedPublisher) &&
          (filters.minScore === undefined || item.score >= filters.minScore) &&
          (!filters.sdk?.length || filters.sdk.includes(this.details.get(item.name)?.requirements.flutterConstraint ? "flutter" : "dart")) &&
          (!filters.platform?.length ||
            filters.platform.some((platform) => {
              const platforms = this.details.get(item.name)?.latestVersion.platforms;
              return platform === "desktop" ? platforms?.some((value) => ["linux", "macos", "windows"].includes(value)) : platforms?.includes(platform);
            }))
        );
      });
  ...
  async getVersion(name: string, version: string) {
    return structuredClone(this.details.get(name)?.versions.find((item) => item.version === version) ?? null);
  }
  async getFiles(name: string, version: string) {
    return (await this.getVersion(name, version)) ? structuredClone(this.details.get(name)?.files ?? []) : null;
  }
  async getFile(name: string, version: string, path: string): Promise<PackageFile | null> {
    return (await this.getFiles(name, version))?.find((item) => item.path === path) ?? null;
  }
  async getScore(name: string, version: string) {
    return (await this.getVersion(name, version)) ? structuredClone(this.details.get(name)?.score ?? null) : null;
  }
  ...
  async listImports() {
    return structuredClone([...this.imports.values()].sort((a, b) => b.createdAt.localeCompare(a.createdAt)));
  }
  async findAccountByUsername(username: string) {
    return structuredClone([...this.accounts.values()].find((item) => item.username === username) ?? null);
  }
  ...
  async publishArchive(archive: Buffer, actor: PublishActor) {
    ...
    if (this.storageDirectory) {
      const directory = resolve(this.storageDirectory);
      await mkdir(directory, { recursive: true });
      const destination = join(directory, `${parsed.name}-${parsed.version}.tar.gz`);
  ...
  private registerParsedArchive(parsed: Awaited<ReturnType<typeof parsePackageArchive>>, archive: Buffer, actor: PublishActor) {
    ...
    const packageVersion = {
      ...
      sdk: {
        ...
        ...(parsed.flutterConstraint ? { flutter: parsed.flutterConstraint } : {}),
      },
    ...
    if (existing) {
      ...
      this.details.set(parsed.name, {
        package: {
          ...
          hasPreview: parsed.files.some((file) => file.preview !== "unsupported"),
        },

...
const version = (
  ...
  daysAgo: number,
  prerelease = false,
  sourceType: "native" | "pubdev_import" = "native",
): PackageVersion => ({
  version: value,
  pubspec: {
    name: "aurora_ui",
  ...
  archiveSha256: `0a4cc3d8f1${value.replace(/\D/g, "").padEnd(54, "e")}`.slice(
    0,
    64,
  ),
  retractedAt: null,
...
const summaries: PackageSummary[] = [
  ...
  },
  {
    ...
    latestVersion: "1.8.0",
    isDiscontinued: false,
    topics: ["flutter", "security", "storage"],
  ...
  },
  {
    ...
    publisherId: "data.internal",
    description: "Unified analytics contract for mobile and web",
    latestVersion: "4.1.2",
    isDiscontinued: false,
    topics: ["dart", "analytics"],
  ...
  {
    name: "legacy_networking",
    publisherId: "platform.internal",
    description: "Deprecated networking facade; migrate to network_core",
    latestVersion: "0.9.8",
    isDiscontinued: true,
    topics: ["dart", "network"],
  ...
  const versions =
    item.name === "aurora_ui"
      ? [
          ...
          version("2.3.0", 17),
          version("2.2.0", 51),
          version("2.4.0-rc.1", 2, true),
          version("2.4.0-beta.1", 4, true),
          version("2.4.0-dev.2", 6, true),
        ]
      : [
          version(
            item.latestVersion,
            8,
            false,
            item.name === "analytics_bridge" ? "pubdev_import" : "native",
          ),
        ];
  demoDetails.set(item.name, {
    ...
    dependencies:
      item.name === "aurora_ui"
        ...
        : [
            {
              name: item.topics.includes("flutter") ? "flutter" : "meta",
              constraint: item.topics.includes("flutter") ? "SDK" : "^1.16.0",
              scope: "dependencies",
              source: item.topics.includes("flutter") ? "sdk" : "hosted",
              registry: item.topics.includes("flutter") ? "sdk" : "pubdev",
            },
          ],
    ...
    files:
      item.name === "aurora_ui" ? structuredClone(files) : files.slice(0, 3),
  });
}

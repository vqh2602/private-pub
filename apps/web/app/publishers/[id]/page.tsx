...
export default async function PublisherPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return <main className="subpage"><section className="subpage-hero"><div className="content-shell publisher-title"><div className="publisher-logo"><Building2 /></div><div><Badge tone="green"><CheckCircle2 size={12} />Verified publisher</Badge><h1>{id}</h1><p>Shared platform capabilities for mobile and web product teams.</p><div className="package-meta"><span><Globe2 size={15} />{id}</span><span><Mail size={15} />platform-team@internal</span></div></div></div></section><section className="content-shell compact-content"><span className="eyebrow">Published packages</span><h2>{packages.length} packages</h2><div className="package-list">{packages.map((item) => <PackageCard key={item.name} item={item} />)}</div></section></main>;
}

...
export default function ImportsPage() {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    if (!file) {
      setError(locale === "en" ? "Please select a .tar.gz package file." : "Vui lòng chọn một file package .tar.gz.");
      return;
    }
    setLoading(true);
    setError("");
    ...
    try {
      ...
      if (!response.ok) throw new Error(payload.message || (locale === "en" ? "Unable to import package." : "Không thể import package."));
      setResult(payload);
    } catch (reason) {
      setError(reason instanceof Error ? reason.message : (locale === "en" ? "Unable to import package." : "Không thể import package."));
    } finally {
      setLoading(false);
    }
  }
    ...
    <div className="content-shell compact-content">
      <div className="page-heading">
        <div>
          ...
          <p>Upload một Dart/Flutter package archive để xác minh và đưa vào private registry.</p>
        </div>
      ...
      <div className="wizard">
        <div className={`wizard-steps ${styles.wizardSteps}`}>
          {steps.map((label, index) => (
            <span key={label} className={activeStep >= index + 1 ? "active" : ""}>
              {index + 1} <i>{label}</i>
            </span>
          ))}
        </div>
        <form onSubmit={submit}>
          <label className={`${styles.archiveDropzone}${file ? ` ${styles.hasFile}` : ""}`}>
            ...
            {file ? <>
              ...
              <span>{formatBytes(file.size)} · {locale === "en" ? "Ready to verify" : "Sẵn sàng xác minh"}</span>
            </> : <>
              <UploadCloud />
              <strong>{locale === "en" ? "Choose a package archive" : "Chọn package archive"}</strong>
              <span>{locale === "en" ? "Only .tar.gz files up to 100 MB are accepted" : "Chỉ chấp nhận file .tar.gz, tối đa 100 MB"}</span>
            </>}
          </label>
          ...
          {loading && (
            <div className="validation-box">
              ...
              <div>
                <strong>{locale === "en" ? "Verifying package" : "Đang xác minh package"}</strong>
                <p>{locale === "en" ? "The archive is saved only after every validation passes." : "Archive chỉ được lưu sau khi vượt qua toàn bộ bước kiểm tra."}</p>
              </div>
            </div>
          )}
          {error && (
            <div className={`validation-box ${styles.validationError}`}>
              <XCircle />
              <div>
                <strong>{locale === "en" ? "Import failed" : "Import thất bại"}</strong>
                <p>{error}</p>
              </div>
            </div>
          )}
              ...
              <div>
                <strong>
                  {result.action === "package_created"
                    ? (locale === "en" ? "New package created" : "Đã tạo package mới")
                    : (locale === "en" ? "New version added" : "Đã thêm version mới")}
                </strong>
                <p>{result.message}</p>
                <Link href={`/packages/${encodeURIComponent(result.packageName)}`}>
          ...
          <div className="form-actions">
            {file && !loading && (
              <button type="button" onClick={() => { selectFile(null); if (inputRef.current) inputRef.current.value = ""; }}>

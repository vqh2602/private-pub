...
const pairs: Array<[string, string]> = [
  ...
  ["Search packages, publishers, or topics…", "Tìm package, nhà phát hành hoặc chủ đề…"],
  ...
  ["Try a broader package name, topic, or publisher.", "Hãy thử tên package, chủ đề hoặc nhà phát hành rộng hơn."],
  ["Control plane", "Bảng điều khiển"],
  ["Registry administration", "Quản trị registry"],
  ["System health, queues, policy, and recent audit activity.", "Tình trạng hệ thống, hàng đợi, chính sách và hoạt động kiểm tra gần đây."],
  ...
  ["Upload a Dart/Flutter package archive to verify and import into the private registry.", "Upload một Dart/Flutter package archive để xác minh và đưa vào private registry."],
  ["Choose file", "Chọn file"],
  ["Verify", "Xác minh"],
  ["Result", "Kết quả"],
  ["Choose a package archive", "Chọn package archive"],
  ["Only .tar.gz files up to 100 MB are accepted", "Chỉ chấp nhận file .tar.gz, tối đa 100 MB"],
  ["Verifying package", "Đang xác minh package"],
  ["Import failed", "Import thất bại"],
  ["Choose again", "Chọn lại"],
  ["Verify & import", "Xác minh & import"],
  ["Check archive structure and safe paths", "Kiểm tra cấu trúc archive và đường dẫn an toàn"],
  ["Read and validate pubspec.yaml", "Đọc và xác minh pubspec.yaml"],
  ["Check package name, semantic version, and SDK constraint", "Kiểm tra tên package, semantic version và SDK constraint"],
  ["Calculate SHA-256 and index files", "Tính SHA-256 và lập chỉ mục file"],
  ["The archive is saved only after every validation passes.", "Archive chỉ được lưu sau khi vượt qua toàn bộ bước kiểm tra."],
  ...
  ["Search, review, and download official Flutter releases for each operating system.", "Tìm kiếm, thống kê và tải các bản Flutter chính thức cho từng hệ điều hành."],
  ["Releases", "Bản phát hành"],
  ["Stable releases", "Các bản stable"],
  ["Beta releases in the archive", "Các bản beta trong kho lưu trữ"],
  ["Search Flutter, Dart SDK, or commit…", "Tìm Flutter, Dart SDK hoặc commit…"],
  ...
  ["Try another keyword or remove the release-channel filter.", "Thử từ khóa khác hoặc bỏ bộ lọc kênh phát hành."],
  ["← Previous page", "← Trang trước"],
  ["Next page →", "Trang sau →"],
  ["Unable to load the Flutter SDK list", "Không thể tải danh sách Flutter SDK"],
  ["The Flutter data source is temporarily unavailable.", "Nguồn dữ liệu Flutter tạm thời không khả dụng."],
  ...
  ["New password (at least 10 characters)", "Mật khẩu mới (tối thiểu 10 ký tự)"],
  ["Confirm new password", "Xác nhận mật khẩu mới"],
  ["Sign out", "Đăng xuất"],
  ["Manage tokens", "Quản lý token"],
  ["Developer access", "Truy cập nhà phát triển"],
  ["You are using a temporary password. Please change your password before performing administrative tasks or creating tokens.", "Bạn đang dùng mật khẩu tạm thời. Hãy đổi mật khẩu trước khi thực hiện thao tác quản trị hoặc tạo token."],
  ["Your current session will end after successfully changing the password.", "Phiên đăng nhập hiện tại sẽ kết thúc sau khi đổi thành công."],
  ["Create and revoke tokens for use with dart pub, CI, or package publishing workflows.", "Tạo và thu hồi token dùng với dart pub, CI hoặc quy trình phát hành package."],
  ...
  ["This list only shows versions published by the current account.", "Danh sách này chỉ hiển thị các version do chính tài khoản hiện tại publish."],
  ["Go to sign in →", "Đi đến đăng nhập →"],
  ["No published releases yet", "Chưa có bản publish nào"],
  ["Packages and versions published with this account's token will appear here.", "Sau khi publish bằng token của tài khoản này, package và version sẽ xuất hiện tại đây."],
  ["Revoked", "Đã thu hồi"],
  ...
  ["The package may be private, renamed, or unavailable.", "Package có thể là riêng tư, đã đổi tên hoặc không khả dụng."],
...
function localized(value: string, locale: Locale) {
  ...
  if (matchingVersionsEn) return locale === "vi" ? `${matchingVersionsEn[1]} phiên bản phù hợp` : value;
  ...
  if (pageEn) return locale === "vi" ? `Trang ${pageEn[1]} / ${pageEn[2]}` : value;

  const latestFlutter = normValue.match(/^Mới nhất: Flutter (.+)$/);
  if (latestFlutter) return locale === "en" ? `Latest: Flutter ${latestFlutter[1]}` : value;
  const latestFlutterEn = normValue.match(/^Latest: Flutter (.+)$/);
  if (latestFlutterEn) return locale === "vi" ? `Mới nhất: Flutter ${latestFlutterEn[1]}` : value;

  const downloadFlutter = normValue.match(/^Tải Flutter (.+)$/);
  if (downloadFlutter) return locale === "en" ? `Download Flutter ${downloadFlutter[1]}` : value;
  const downloadFlutterEn = normValue.match(/^Download Flutter (.+)$/);
  if (downloadFlutterEn) return locale === "vi" ? `Tải Flutter ${downloadFlutterEn[1]}` : value;

  const pair = pairs.find(([en, vi]) => normalizeSpace(en) === normValue || normalizeSpace(vi) === normValue);
...
function translateDOM(root: HTMLElement | Node, locale: Locale) {
  ...
  const elements = element.querySelectorAll<HTMLElement>("[placeholder], [title], [aria-label]");

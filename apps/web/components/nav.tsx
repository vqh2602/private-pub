...
import { Box, KeyRound, Menu, Moon, Search, ShieldCheck, Sun, UserRound, X } from "lucide-react";
  ...
  useEffect(() => {
    ...
    fetch("/api/auth/me", { cache: "no-store", signal: controller.signal })
      ...
      .catch((reason: unknown) => {
        if (!(reason instanceof DOMException && reason.name === "AbortError")) setUser(null);
      });
  ...
  return <header className="topbar">
    <div className="nav-shell">
      ...
      <div className="nav-actions">
        ...
        <Link href="/tokens" className="profile"><span>{user ? <UserRound size={15} /> : <KeyRound size={15} />}</span><div><strong>{user?.username ?? "Tài khoản"}</strong><small>{user ? roleLabel(user.role) : "Đăng nhập &amp; token"}</small></div></Link>

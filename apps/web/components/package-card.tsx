...
export function PackageCard({ item }: { item: PackageSummary }) {
  return <article className="package-card">
    ...
    <div className="package-stats">
      <span><Star size={15} /><GitHubStars repositoryUrl={item.repository} /></span><span><Download size={15} />{item.downloads30d.toLocaleString()}</span>{item.hasPreview && <span><Eye size={15} />Preview</span>}

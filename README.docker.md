...
_Lưu ý: Quá trình build lần đầu tiên sẽ mất khoảng vài phút do Docker cần tải môi trường Node.js và thiết lập Flutter SDK bên trong container._
...
_(Gõ `y` và nhấn Enter khi hệ thống yêu cầu xác nhận)_
...
- **Khắc phục:** Nếu bạn thay đổi mật khẩu admin và quên, hoặc database khởi tạo lỗi, hãy chạy lệnh reset database ở phần trên, hoặc xóa hoàn toàn Docker Volume chứa dữ liệu Postgres để Docker dựng lại database trống từ đầu:
  ...
  ```
  _(Lưu ý: Lệnh này sẽ xóa sạch dữ liệu packages đã upload của bạn)_
...
- Để xem log trực tiếp theo thời gian thực (ví dụ service API):
  ...
  ```
  _(Thay `api` bằng `web`, `worker`, `postgres`, hoặc `redis` tùy ý)_
